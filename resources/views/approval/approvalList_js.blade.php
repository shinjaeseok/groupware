<script>
    ...
    // Ajax URL
    const approvalListUrl = "/approval/approvalList";
    ...
    function doSearch(table_id = 1, type = 'all') {
        ...
        let oTable = $('#dataTables_' + table_id).DataTable({
            ...
            ajax: {
                type: "GET",
                url: "/approval/approvalList/list/" + type,
                dataType: "json",
                cache: false,
                async: false,
                dataSrc: '',
            ...
            columns: [
                ...
                { data: "title", className: "text-left" ,
                    render: function(data, type, row, meta) {
                        return data + ` <i class="fa fa-external-link" style="cursor: pointer;" onclick="view_detail(${row.approval_id})"></i>`;
                    }
                },
    ...
    function view_detail(id) {
        ...
        $.ajax({
            ...
            $('#btn_approval_deny').show();
            $('#btn_approval_admit').show();
            $('#btn_approval_cancel').hide();
            $('#comment_textarea').show();

            if (result.data.auth_user_approval_line_data.approval_status == '완료') {
                $('#btn_approval_deny').hide();
                $('#btn_approval_admit').hide();
    ...
    function commentDelete(id) {
        $.ajax({
            ...
            url: approvalUrl + "/comment/delete",
            dataType: 'json',
            cache: false,
            async: false,
        }).done(function (result) {
    ...
    $("#btn_approval_admit").on("click", function () {
        $('#modal_5').modal('show');
        $('#modal_id_5').val('approvalLineStateChangeAdmit');
    });

    $("#btn_approval_deny").on("click", function () {
        $('#modal_5').modal('show');
        $('#modal_id_5').val('approvalLineStateChangeDeny');
    });

    $("#btn_approval_cancel").on("click", function () {
        $('#modal_5').modal('show');
        $('#modal_id_5').val('approvalLineStateChangeCancel');
    });
