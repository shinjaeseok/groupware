<script>
    ...
    $('#password_check').on("click", function (){
        ...
        var process_mode = '/password_check';
        var password = $('#password').val();

        if (!password) {
            toastr["error"]("기존 비밀번호를 입력해주세요.",'오류');
            return false;
        }
    ...
    $("#btn_profile_file").on('click', function (){
        $("#modal_2").modal("show");
    });


    // 저장
    $("#btn_modal_profile_file_save").click(function () {
        var process_mode = "/profile_file_update";

        // 첨부파일 처리
        var formData = new FormData( $("#form_modal_2")[0] );
